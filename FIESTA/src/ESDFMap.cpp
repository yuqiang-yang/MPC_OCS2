...
//add by yq
void fiesta::ESDFMap::SaveMap(std::string filename)
{
  std::ofstream fp(filename, std::fstream::out);//只写文件 + trunc若文件存在则删除后重建
  std::ofstream fp_data(filename + "_data", std::fstream::out);//只写文件 + trunc若文件存在则删除后重建

  if (!fp.is_open() || !fp_data.is_open())
  {
    std::cerr << "cannot open file" << std::endl;
    return;
  }
  fp << origin_(0) << " " << origin_(1) << " " << origin_(2) << std::endl;
  fp << reserved_idx_4_undefined_ << std::endl;
  fp << grid_total_size_ << " " << grid_size_yz_<< std::endl << std::endl;
  fp << resolution_ << " " << resolution_inv_  << std::endl;
  fp << total_time_ << std::endl;
  fp << max_vec_(0) << " " << max_vec_(1) << " " << max_vec_(2)  << std::endl;
  fp << min_vec_(0) << " " << min_vec_(1) << " " << min_vec_(2)  << std::endl;
  fp << last_max_vec_(0) << " " << last_max_vec_(1) << " " << last_max_vec_(2)  << std::endl;
  fp << last_min_vec_(0) << " " << last_min_vec_(1) << " " << last_min_vec_(2)  << std::endl;
  fp.close();


  // for debug 
  occupancy_buffer_[1000] = 100;
  prev_[1000] = 200;
  closest_obstacle_[2000](1) = 300;
  fp_data.write(reinterpret_cast<char*>(&occupancy_buffer_[0]), occupancy_buffer_.size() * sizeof(double));
  fp_data.write(reinterpret_cast<char*>(&distance_buffer_[0]), distance_buffer_.size() * sizeof(double));
  fp_data.write(reinterpret_cast<char*>(&closest_obstacle_[0]), closest_obstacle_.size() * sizeof(int) *3);
  fp_data.write(reinterpret_cast<char*>(&num_hit_[0]), num_hit_.size() * sizeof(int));
  fp_data.write(reinterpret_cast<char*>(&num_miss_[0]), num_miss_.size() * sizeof(int));
  fp_data.write(reinterpret_cast<char*>(&head_[0]), head_.size() * sizeof(int));
  fp_data.write(reinterpret_cast<char*>(&prev_[0]), prev_.size() * sizeof(int));
  fp_data.write(reinterpret_cast<char*>(&next_[0]), next_.size() * sizeof(int));

  fp_data.close();

}
void fiesta::ESDFMap::LoadMap(std::string filename)
{
  std::ifstream fp_data(filename + "_data");
  ...
  //please check the order when saving the map.
  Eigen::Vector3d origin;
  fp >> origin(0);
  fp >> origin(1);
  fp >> origin(2);

  int reserved_idx_4_undefined;
  fp >> reserved_idx_4_undefined;

  int grid_total_size, grid_size_yz;
  fp >> grid_total_size;
  fp >> grid_size_yz;
  ...
  //check the loaded map and current map
  if(!(origin(0) == origin_(0) && origin(1) == origin_(1) && origin(2) == origin_(2) &&
  reserved_idx_4_undefined_ == reserved_idx_4_undefined && grid_total_size == grid_total_size_&&
  grid_size_yz == grid_size_yz_ && resolution == resolution_))
  {
    std::cerr << "The map to be loaded is not the type of current map(properties). Check the size and the resolution." << std::endl;
    
    std::cerr << "origin_" << origin_.transpose() << " " << "reserved_idx_4_undefined_" << reserved_idx_4_undefined_ << " " <<
    "grid_total_size_" << grid_total_size_ << " resolution_" << resolution_ << std::endl;
    return;
  }
  fp >> total_time_;

  // I guess other settings is the same.
 
  std::cerr << "origin " << origin.transpose() << "reserved_idx_4_undefined_ " << reserved_idx_4_undefined_ << std::endl;

  fp_data.read(reinterpret_cast<char*>(&occupancy_buffer_[0]), occupancy_buffer_.size() * sizeof(double));
  fp_data.read(reinterpret_cast<char*>(&distance_buffer_[0]), distance_buffer_.size() * sizeof(double));
  fp_data.read(reinterpret_cast<char*>(&closest_obstacle_[0]), closest_obstacle_.size() * sizeof(int) *3);
  fp_data.read(reinterpret_cast<char*>(&num_hit_[0]), num_hit_.size() * sizeof(int));
  fp_data.read(reinterpret_cast<char*>(&num_miss_[0]), num_miss_.size() * sizeof(int));
  fp_data.read(reinterpret_cast<char*>(&head_[0]), head_.size() * sizeof(int));
  fp_data.read(reinterpret_cast<char*>(&prev_[0]), prev_.size() * sizeof(int));
  fp_data.read(reinterpret_cast<char*>(&next_[0]), next_.size() * sizeof(int));

}

void fiesta::ESDFMap::SetAllVoxelFree()
{
    for (int x = min_vec_(0); x <= max_vec_(0); ++x)
    for (int y = min_vec_(1); y <= max_vec_(1); ++y)
    for (int z = min_vec_(2); z <= max_vec_(2); ++z) {
