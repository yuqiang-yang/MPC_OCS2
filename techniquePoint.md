...
### 专利
+ 现有技术
    + 场景：非结构动态环境下的本体与障碍感知 图：桌面加人动态调整物体的gif  
        技术：点云转化Octomap再进行凸包分解，基于凸包的遍历式碰撞检测，凸包运动碰撞点计算与局部线性化，耗时且缺乏环境全局梯度信息用于最优轨迹寻找 图，去moveit官网看看，放一下FCL的，放一下trajOpt的
        问题：无法实现本体与环境动态情况下的精确感知与环境点云滤除，步骤繁琐且前端搜索与后端优化地图可重用性不高，建图的时间与空间复杂度随分辨率的提高而指数上涨
    + 场景：高维可行空间下实时（重）规划 图：待定。放一些规划的预测与修正的图
        技术：几何式轨迹采样或搜索算法，图RRT或PRM
        问题：未考虑微分约束，无法用于具有非完整约束的底盘，未考虑系统模型或未与系统模型紧耦合，
    + 场景：冗余移动机械臂整身灵巧操作  图：放整身操作的gif
        技术：冗余机械臂整身建模，分体规划或层级规划，可操作度优化
        问题：大大损失了解空间，复杂环境下难以求得可行解，不具有概率完备性，也无法求得全局最优解，执行效率慢。
+ 本发明技术方案
+ 核心保护点
    + 动态环境基于滚动预测的拓扑引导式实时重规划方法->先进性
    + DDP-based反馈最优控制？
    + 本体与环境增量建图方法
    + 整身灵巧操作->易取证，分体与整身效果明显，灵巧的jerk易看出
+ 技术效果
+ 可规避性与可取证性