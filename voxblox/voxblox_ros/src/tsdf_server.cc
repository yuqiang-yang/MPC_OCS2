...
TsdfServer::TsdfServer(const ros::NodeHandle& nh,
  ...
  if (update_mesh_every_n_sec > 0.0) {
    update_mesh_timer_ =
        ...
        ROS_INFO_STREAM("debugging successfully start the mesh timer"<< nh_private_.getNamespace() << std::endl);
  }
  ...
  if (publish_map_every_n_sec > 0.0) {
    publish_map_timer_ =
        ...
        ROS_INFO_STREAM("debugging successfully start the map timer"<< nh_private_.getNamespace() << std::endl);
  }
}
...
bool TsdfServer::getNextPointcloudFromQueue(
  ...
  if (queue->empty()) {
    if(verbose_) std::cerr << "point cloud is empty!" << std::endl;
    return false;
  }
  *pointcloud_msg = queue->front();
  if(verbose_) std::cerr << "TsdfServer::getNextPointcloudFromQueue" << std::endl;

  if (transformer_.lookupTransform((*pointcloud_msg)->header.frame_id,
                                   world_frame_,
                                   (*pointcloud_msg)->header.stamp, T_G_C)) {
    if(verbose_) std::cerr << "TsdfServer::getNextPointcloudFromQueue lookupTransform success" << std::endl;
...
void TsdfServer::insertPointcloud(
  ...
  if (pointcloud_msg_in->header.stamp - last_msg_time_ptcloud_ >
      ...
      if(verbose_) std::cerr << "receive point cloud callback!" << std::endl;
  ...
  while (
      getNextPointcloudFromQueue(&pointcloud_queue_, &pointcloud_msg, &T_G_C)) {
        if(verbose_) std::cerr << "insert the point cloud!" << std::endl;
...
void TsdfServer::updateMesh() {
  if (verbose_) {
    ...
    ROS_INFO_STREAM("Updating mesh." << nh_private_.getNamespace());
  }
...
void TsdfServer::updateMeshEvent(const ros::TimerEvent& /*event*/) {
  if(verbose_)
    ROS_INFO_STREAM("Updating mesh mesh mesh." << nh_private_.getNamespace());
...
void TsdfServer::publishMapEvent(const ros::TimerEvent& /*event*/) {
  if(verbose_)
    ROS_INFO_STREAM("Updating map event." << nh_private_.getNamespace());
